...
public class CountAndSay {
    ...
    public static void main(String[] args) {
        String currentByPrevious = new CountAndSay().getCurrentByPrevious("1211");
        System.out.println(currentByPrevious);
    ...
    public String countAndSay(int n) {
        ...
        String temp = "1";
        for (int i = 1; i < n; i++) {
            temp = getCurrentByPrevious(temp);
        }
        return String.valueOf(temp);
    }

    //21  1211
    public String getCurrentByPrevious(String s) {
        String[] chars = s.split("");
        StringBuilder append = new StringBuilder();
        String tempAppend = chars[0];
        for (int i = 1; i < chars.length; i++) {
            if (!chars[i].equals(chars[i - 1])) {
                append.append(tempAppend.length()).append(tempAppend.charAt(0));
                tempAppend = chars[i];
            }else {
                tempAppend += chars[i];
            }
